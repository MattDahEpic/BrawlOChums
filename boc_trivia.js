...
module.exports = function (messageRaw,message, ws) {
    ...
    if ((message.trivia === "loadquestions" || message.trivia === "hidequestion" || message.trivia === "displayquestion") && ws.clientType === "game") {
        const clients = s.connections.get(ws.gameCode).clients.values();
        for (let i = 0; i < clients.length; i++) {
            if (clients[i] !== null) clients[i].send(messageRaw);
        }
